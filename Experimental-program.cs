...
namespace Neural_network
{
    class Program
    {
        ...
        // with the issue described above, I implemented the code needed for calculating the average cost, but it doesn't seem to have made a difference
            ...
            for (int i = 0; i < pastCostsLength; i++)
            {
                pastCosts.Add(100);
            }
            for (int i = 0; i < inputLayer.Length; i++)
            ...
            for (int a = 2; a < trainingData.Length-1; a++)
            {
                ...
                pastCosts.RemoveAt(0);
                pastCosts.Add(cost);
                float averageCost = 0.0f;
                for (int i = 0; i < pastCostsLength; i++)
                {
                    averageCost += pastCosts[i];
                }
                averageCost -= (pastCosts.Min()+pastCosts.Max());
                averageCost /= (pastCostsLength-2);
                for (int i = 0; i < inputLayer.Length; i++)
                {
                    inputLayer[i].weightStep(direction);
                }
                ...
                {
                    Console.WriteLine("Current iteration: "+a+" | Current cost: "+cost+" | Current average: "+averageCost);
                }
                else
                {
                    Console.WriteLine(cost + " "+averageCost);
                }
                if (Math.Abs(averageCost) < Math.Abs(bestCost))
                {
